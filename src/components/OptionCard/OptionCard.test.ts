...
describe("optionCard", () => {
  it("renders a native radio associated with its label", () => {
    const html = optionCard({ text: "A", index: 2, answered: false, checked: false, state: "idle" });
  ...
  it("escapes option text that contains literal markup", () => {
    const html = optionCard({ text: "<button>", index: 0, answered: false, checked: false, state: "idle" });
    expect(html).toContain("&lt;button&gt;");
    expect(html).not.toContain("<button>");
  });

  it("disables and adds a describedby status to the correct option once answered", () => {
    const html = optionCard({ text: "A", index: 0, answered: true, checked: true, state: "correct" });
  ...
  it("labels the chosen wrong option", () => {
    const html = optionCard({ text: "A", index: 1, answered: true, checked: true, state: "incorrect" });
